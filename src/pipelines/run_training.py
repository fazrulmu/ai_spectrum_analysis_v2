...
from data.data_processing import prepare_ir_dataset, prepare_uv_dataset
from modeling.modeling import train_base_model_cv
...
def execute_training(args, config):
    ...
    def _train_base_stage(config, spectrum_type):
        ...
        # Memuat dataset
        if spectrum_type == 'ir':
            X_spec, X_meta, y, groups = prepare_ir_dataset(config)
        else:
            X_spec, X_meta, y, groups = prepare_uv_dataset(config)
        
        # Melatih model dasar menggunakan cross-validation dan mendapatkan prediksi OOF
        ...
        # --- PERBAIKAN: Pastikan model final dilatih ulang dan disimpan ---
        # Fungsi train_base_model_cv sudah melakukan ini di akhir, jadi kita hanya perlu
        # memastikan alur ini dipanggil dengan benar. Baris ini secara eksplisit
        # menegaskan bahwa tahap ini selesai dan model seharusnya sudah tersimpan.
        model_path = os.path.join(paths['saved_models_dir'], f'base_model_{spectrum_type}_final.keras')
        if os.path.exists(model_path):
            print(f"✅ Model dasar final {spectrum_type.upper()} berhasil disimpan di {model_path}")
        else:
            print(f"⚠️ Peringatan: Model dasar final {spectrum_type.upper()} tidak ditemukan setelah pelatihan.")
    ...
    def _train_meta_stage(config):
        ...
        meta_model = XGBClassifier(objective='binary:logistic', eval_metric='logloss', n_estimators=250, random_state=config['modeling']['random_state'])
