# train_ensemble.py (Versi BARU dengan target gabungan IR & UV)
...
def main():
    print("🚀 Memulai alur kerja pelatihan ensemble dengan TARGET GABUNGAN...")
    ...
    print("\n[Tahap 1/5] Memuat dan memproses dataset mentah...")
    X_spec_ir, X_meta_ir, y_ir, groups_ir = prepare_ir_dataset(config)
    X_spec_uv, X_meta_uv, y_uv, groups_uv = prepare_uv_dataset(config)
    
    # 3. Menyelaraskan Sampel (Sama seperti sebelumnya)
    print("\n[+] Menyelaraskan sampel antara dataset IR dan UV...")
    ...
    print(f"Ditemukan {len(merged_df)} sampel yang memiliki data IR dan UV yang cocok.")
    if len(merged_df) == 0:
    ...
    # Filter data X dan grup
    X_spec_ir_aligned = X_spec_ir[ir_indices]
    X_meta_ir_aligned = X_meta_ir[ir_indices]
    X_spec_uv_aligned = X_spec_uv[uv_indices]
    X_meta_uv_aligned = X_meta_uv[uv_indices]
    groups_aligned = merged_df['cas'].values

    # =================================================================
    # == PERUBAHAN: Buat target y_aligned yang berisi label IR dan UV ==
    # =================================================================
    y_ir_aligned = y_ir[ir_indices]
    y_uv_aligned = y_uv[uv_indices]
    y_aligned = np.concatenate([y_ir_aligned, y_uv_aligned], axis=1)
    print(f"Target label gabungan dibuat dengan shape: {y_aligned.shape}")
    # =================================================================
    
    # 4. Latih Model Dasar (Tidak ada perubahan di bagian ini)
    print("\n[Tahap 2/5] Melatih model dasar IR dengan Cross-Validation...")
    ir_oof_preds = train_base_model_cv(X_spec_ir_aligned, X_meta_ir_aligned, y_ir_aligned, groups_aligned, config, 'ir')
    
    print("\n[Tahap 3/5] Melatih model dasar UV dengan Cross-Validation...")
    uv_oof_preds = train_base_model_cv(X_spec_uv_aligned, X_meta_uv_aligned, y_uv_aligned, groups_aligned, config, 'uv')

    # 5. Latih Meta-Model dengan target gabungan
    print("\n[Tahap 4/5] Membuat dataset meta dan melatih meta-model (XGBoost)...")
    ...
    meta_model = XGBClassifier(
        ...
        colsample_bytree=0.8, random_state=config['modeling']['random_state']
    )
    wrapper_model = MultiOutputClassifier(meta_model)
    wrapper_model.fit(X_meta_features, y_aligned) # <-- Melatih dengan y_aligned yang baru
    
    meta_model_path = os.path.join(paths['saved_models_dir'], 'meta_model_final.joblib')
    joblib.dump(wrapper_model, meta_model_path)
    print(f"✅ Meta-model disimpan di {meta_model_path}")
    
    # 6. Evaluasi Meta-Model dengan target gabungan
    print("\n[Tahap 5/5] Mengevaluasi performa meta-model pada prediksi OOF...")
    y_pred_meta = wrapper_model.predict(X_meta_features)
    
    # ====================================================================
    # == PERUBAHAN: Gabungkan nama kelas dari kedua binarizer untuk laporan ==
    # ====================================================================
    ...
    report = classification_report(
        y_aligned, y_pred_meta, target_names=combined_class_names, zero_division=0
    )
    ...
    with open(report_path, 'w') as f:
        f.write("Laporan Kinerja Ensemble Model Gabungan (IR & UV)\n")
        f.write(report)
